...
{
	for (uint8_t i = 0; i < ARRAYSIZE(s_regs); i++)
	{
...
{
	reg_cache_w(OPM_REG_TEST_MODE, en ? 0x01 : 0x00);
}

void xb_opm_set_lfo_reset(bool en)
{
	reg_cache_w(OPM_REG_TEST_MODE, en ? 0x02 : 0x00);
}

void xb_opm_set_key_on(uint8_t channel, uint8_t sn)
{
	reg_cache_w(OPM_REG_KEY_ON, channel | (sn << 3));
}
...
{
	reg_cache_w(OPM_REG_CLKA_UPPER, period >> 2);
	reg_cache_w(OPM_REG_CLKA_LOWER, period & 0x03);
}

void xb_opm_set_clkb_period(uint8_t period)
{
	reg_cache_w(OPM_REG_CLKB, period);
}

void xb_opm_set_timer_flags(XBOpmTimerFlag flags)
{
	reg_cache_w(OPM_REG_TIMER_FLAGS, flags);
}
...
void xb_opm_set_d1t_mul(uint8_t channel, uint8_t op, uint8_t dt1, uint8_t mul)
{
